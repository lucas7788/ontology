...
func getGlobalParam(native *native.NativeService, contract common.Address) (*GlobalParam, error) {
	globalParamBytes, err := native.CacheDB.Get(utils.ConcatKey(contract, []byte(GLOBAL_PARAM)), native.Height)
	if err != nil {
...
func getConfig(native *native.NativeService, contract common.Address) (*Configuration, error) {
	...
	configBytes, err := native.CacheDB.Get(utils.ConcatKey(contract, []byte(VBFT_CONFIG)), native.Height)
	if err != nil {
...
func getPreConfig(native *native.NativeService, contract common.Address) (*PreConfig, error) {
	...
	preConfigBytes, err := native.CacheDB.Get(utils.ConcatKey(contract, []byte(PRE_CONFIG)), native.Height)
	if err != nil {
